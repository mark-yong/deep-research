...
## Homelab Setup (MiniMax + SearXNG)

Fork configured for homelab inference stack (MiniMax M2.7 via SGLang + SearXNG MCP).

### Prerequisites

- MiniMax M2.7 NVFP4 (`m27sg`) running on SGLang at `http://192.168.68.104:8010/v1`
- SearXNG MCP server at `http://192.168.68.104:8080/mcp`

### Installation

```bash
cd ~/projects/deep-research
uv venv
...
Or run from project directory with venv activated:
...
export MINIMAX_BASE_URL=http://192.168.68.104:8010/v1
export SEARXNG_MCP_URL=http://192.168.68.104:8080/mcp
```

### Architecture

```
OpenCode → deep_research (MCP tool) → LangGraph agent
...
All agent settings are in `src/open_deep_research/configuration.py`:

| Setting | Default | Description |
|---|---|---|
| `search_api` | `searxng` | Search provider |
| `research_model` | `minimax:m27sg` | Research sub-agent model |
| `max_researcher_iterations` | `10` | Supervisor cycles (self-hosted = higher budget) |
| `max_concurrent_research_units` | `3` | Parallel sub-agents per cycle |
| `allow_clarification` | `false` | Skip user clarification (programmatic use) |
